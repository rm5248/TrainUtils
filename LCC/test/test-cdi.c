...
static int compare_string_ignore_quote(const char* str1, const char* str2, int len){
    int val = 0;
...
static int control_cdi_offset(int offset){
    const char* xml = "<?xml version='1.0'?>"
            "<cdi xmlns:xsi='http://www.w3.org/2001/XMLSchema-instance\" xsi:noNamespaceSchemaLocation='http://openlcb.org/schema/cdi/1/1/cdi.xsd'>"
            "<identification>"
            "<manufacturer>An NMRA MFG</manufacturer>"
    ...
    char buffer[65];
    buffer[64] = 0;
    lcc_cdi_control_to_xml_at_offset(&cdi_control, offset, buffer);

    int is_same = compare_string_ignore_quote(buffer, xml + offset, 64);
...
int main(int argc, char** argv){
    ...
    }else if(strcmp(argv[1], "control-xml-offset-0") == 0){
        return control_cdi_offset(0);
    }else if(strcmp(argv[1], "control-xml-offset-128") == 0){
        return control_cdi_offset(128);
    }

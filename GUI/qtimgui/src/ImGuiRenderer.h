...
private:
    ...
    // NOTE(TrainUtils): these must be initialized.  They are copied straight
    // into io.MouseWheel/MouseWheelH on the first frame, and a garbage value
    // there is read as a scroll gesture -- which, with imgui-node-editor,
    // leaves the canvas at a random zoom before the user has touched anything.
